...
const classifyTopic = (quickProvider, quickModel, messages, availableModes, maxTokens, currentMode, callback) => {
	...
	const handleResponse = (err, text) => {
		if (err) {
			log('warn', `[Classifier] Request failed: ${err.message}`);
			callback(err, null);
			return;
		}
		...
		if (!result) {
			log('warn', `[Classifier] Failed to parse result from: ${(text || '').substring(0, 200)}`);
			callback(new Error('Failed to parse classification result'), null);
			return;
		}

		log('info', `[Classifier] Result: isNewTopic=${result.isNewTopic}, mode="${result.mode}"`);
