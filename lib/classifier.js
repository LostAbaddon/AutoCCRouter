...
const SESSIONPLOTCACHE = new Map();
...
const sendAnthropicClassification = (provider, body, maxTokens, callback) => {
	...
	const path = pathPrefix.match(/v\d\/(chat\/|messages\/)/) ? pathPrefix : `${pathPrefix}/v1/messages`;
...
// 发送 OpenAI 格式的分类请求
const sendOpenAIClassification = (provider, model, body, callback) => {
	const _origApiKey = provider.apiKey;
	const acquired = acquireKey(provider._name, _origApiKey);
	const key = acquired.key;
	...
	const openaiBody = buildOpenAIRequest(body, provider._name, model);
	const baseUrl = provider.baseUrl.replace(/\/+$/, '');
	const targetUrl = new URL(baseUrl);
...
// 将信息提交给模型并进行后续处理的实际调用者
// callback(err, result)  result = { isNewTopic: boolean, mode: string } 或 null
const classifyTopic = (quickProvider, quickModel, messages, sessionId, availableModes, maxTokens, currentMode, conversationGroups, callback) => {
	...
	if (plots?.length) {
		...
		log('debug', `[Classifier] Trim By Topic: ${cleanMessages.length} → ${filted.length} messages (${SESSIONPLOTLIMIT} topics)`);
		cleanMessages = filted;
	}
	const maxGroupCount = conversationGroups > 0 ? conversationGroups : SESSIONMESSAGELIMIT;
	if (cleanMessages.length > 0) {
		...
		log('debug', `[Classifier] Trimmed: ${messages.length} → ${cleanMessages.length} messages (${maxGroupCount} groups)`);
	}
	else {
		callback(null, {isNewTopic: true, mode: "quick"});
		return;
	...
	log('debug', `[Classifier] Sending to ${quickProvider._name}:${quickModel}, modes=[${availableModes.map((m) => m.name).join(',')}]`);
	classifierIndex ++;
	...
	const handleResponse = (err, text) => {
		...
		if (!result) {
			...
			if (result.isNewTopic) {
				...
				if (!has) plots.push([Date.now(), isUser, last]);
			}
			// 缓存分析结果
			const cache = {
				timestamp: Date.now(),
	...
	if (quickProvider.type === 'gemini') {
		...
		sendOpenAIClassification(quickProvider, quickModel, classificationBody, handleResponse);
	}
	else {
		// anthropic 及其他
