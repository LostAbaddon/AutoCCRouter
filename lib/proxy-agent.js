...
const proxyRequest = (proxyUrl, options, requestBody, callback) => {
	...
	const timedCallback = (err, proxyRes) => {
		const elapsed = Date.now() - startTime;
		const host = options.hostname || 'unknown';
		if (err) {
			log('info', `[Timing] ${host} request failed after ${elapsed}ms: ${err.message}`);
		}
		else {
			log('info', `[Timing] ${host} responded in ${elapsed}ms, status=${proxyRes ? proxyRes.statusCode : '?'}`);
		}
	...
	if (!proxyUrl) {
		...
		const req = transport.request(options, (proxyRes) => {
			timedCallback(null, proxyRes);
		});
		req.on('error', (err) => { timedCallback(err); });
		if (requestBody) {
	...
	connectReq.on('connect', (res, socket) => {
		...
		if (res.statusCode !== 200) {
			socket.destroy();
			timedCallback(new Error(`Proxy CONNECT failed with status ${res.statusCode}`));
			return;
		}
	...
	connectReq.on('timeout', () => {
		...
		connectReq.destroy();
		timedCallback(new Error('Proxy CONNECT timeout'));
	});
