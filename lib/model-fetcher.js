...
const fetchOpenAIModels = async (provider) => {
	...
	const result = await fetchUrl(url, provider.proxy, key).finally(() => settleKey({ isSuccess: true }));
...
const fetchGeminiModels = async (provider) => {
	...
	log('debug', `[ModelFetcher] Fetching Gemini models from ${url.replace(key, '***')}`);

	const result = await fetchUrl(url, provider.proxy, null).catch((e) => { settleKey({ isKeyFailure: true }); throw e; });
