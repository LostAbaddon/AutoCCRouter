...
const prepareModelMatchRules = (modelMapping) => {
	// 预编译每条规则的 matcher，并按优先级排序：精确前缀在前、通配规则在后；同组内按 prefix 字符串长度降序
...
const mapModel = (modelName, modelMapping) => {
	...
	const matchRules = prepareModelMatchRules(modelMapping);
	for (const { rule, matcher } of matchRules) {
...
const matchModelsByRule = (modelName, modelMapping) => {
	if (!modelName || !modelMapping || modelMapping.length === 0) {
		return null;
	}
...
module.exports = { mapModel, matchModelsByRule, isWildcardPrefix };
