...
const selectModel = (modelsArray) => {
	...
	for (const spec of modelsArray) {
		const parsed = typeof spec === 'string' ? parseModelSpec(spec) : {
			providerName: spec.provider._name,
			model: spec.model,
		};
		if (!parsed) {
			continue;
	...
	const weighted = candidates.map((c) => {
		const w = c.linkWeight * (maxDone + 5) / (c.numDone + 5) * (maxDoing + 1) / (c.numDoing + 1);
		return { ...c, weight: w };
	});
	log('info', `[model-router] candidate weights:`);
	for (const w of weighted) {
		log('info', `[model-router] - ${w.providerName}/${w.model}: ${w.weight.toFixed(2)} (${w.linkWeight.toFixed(2)}, ${w.numDone}, ${w.numDoing})`);
	}
	...
	for (const w of weighted) {
		...
		if (target < cumulative) {
			log('info', `[model-router] target: ${w.providerName}/${w.model}`);
			return { providerName: w.providerName, model: w.model, spec: w.spec };
		}
	}
	const last = weighted[weighted.length - 1];
	log('info', `[model-router] target: ${last.providerName}/${last.model}`);
