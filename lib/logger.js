...
const logClientStage = (client, interactionId, stage, label, data) => {
	...
	try {
		...
		log('info', `[${client.charAt(0).toUpperCase() + client.slice(1)}-Log] #${interactionId} stage=${stage}(${label}) → ${filename}`);
	}
	catch (e) {
...
const logCodexStage = (interactionId, stage, label, data) => {
	logClientStage('codex', interactionId, stage, label, data);
};

module.exports = { log, getNextInteractionId, logClientStage, logCodexStage };
