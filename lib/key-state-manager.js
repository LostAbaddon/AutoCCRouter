...
function releaseKey(providerName, key, opts = {}) {
	...
	if (opts.isSuccess) {
		...
		// 成功调用即表示该 provider 当前可用,清掉 isProviderDown 状态
		// 否则一旦置 down 后续再无业务机会重置,导致下游(model-router 等)读到的永远是 down
		const pState = providerStates.get(providerName);
		if (pState) {
			pState.isProviderDown = false;
		}
	}
}
...
/**
 * 获取单个 provider 的状态({ isProviderDown, lastErrorAt })
 * 不存在时返回 null。
 */
function getProviderState(providerName) {
	...
	return providerStates.get(providerName) || null;
}
