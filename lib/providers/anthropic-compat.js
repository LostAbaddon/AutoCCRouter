...
const restoreThinkingBlocks = (body, provider) => {
	if (!body) {
		return body;
	}
	let parsed;
	...
	// 对于非 Anthropic 的模型，调整 adaptive 策略
	if (provider !== 'anthropic' && provider.thinking?.type === 'adaptive') {
...
const forwardRequest = (provider, req, body, selectedKey, callback) => {
	...
	// 处理 Thinking 块和配置
	let processedBody = restoreThinkingBlocks(body, provider._name);
	try {
...
const handleAnthropicRequest = (provider, targetModel, originalModel, req, body, res, onComplete) => {
	...
	forwardRequest(provider, req, body, selectedKey, (err, proxyRes) => {
		if (err) {
