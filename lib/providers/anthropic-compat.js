			...
			for (let i = 0; i < lines.length; i++) {
				...
				if (parsed.type === 'message_start' && parsed.message) {
					...
					if (usageMeta && usageMeta.interactionId) {
						logClientStage('claude', usageMeta.interactionId, '3', 'response', { note: 'streaming-sse', promptTokens: streamInputTokens, completionTokens: streamOutputTokens });
						logClientStage('claude', usageMeta.interactionId, '4', 'result', { note: 'streaming-sse-to-anthropic', promptTokens: streamInputTokens, completionTokens: streamOutputTokens });
					}
				}
			}
...
const forwardRequest = (provider, req, body, callback) => {
	...
	if (req && req.interactionId) {
		try {
			logClientStage('claude', req.interactionId, '2', 'upstream', JSON.parse(processedBody));
		}
		catch (e) {
			logClientStage('claude', req.interactionId, '2', 'upstream', processedBody);
		}
	}
		...
		if (isStream) {
			...
			proxyRes.on('end', () => {
				log('debug', `[Anthropic] Response body (${responseBody.length}b): ${responseBody.substring(0, 500)}`);
					...
					if (req && req.interactionId) {
						logClientStage('claude', req.interactionId, '3', 'response', parsed);
					}
					...
					if (parsed.usage) {
						const originalUsage = { ...parsed.usage };
						parsed.usage = stripExtraUsage(parsed.usage);
					...
					if (req && req.interactionId) {
						logClientStage('claude', req.interactionId, '4', 'result', parsed);
					}
