...
const resolveAgent = (config, mode, agentSet) => {
	...
	const specValue = models[Math.floor(Math.random() * models.length)];
	if (models.length > 1) {
		log('debug', `[Auto] Random pick from models array for mode=${mode}: ${specValue}`);
	}
	...
	if (!provider._name) {
		Object.defineProperty(provider, '_name', { value: spec.providerName, writable: true, enumerable: false, configurable: true });
	}
...
const handleUserInput = (config, agentSet, sessionKey, originalModel, req, body, res, clientSource) => {
	...
	if (quickModels.length === 0) {
		...
		log('info', `[Auto] Quick empty models → default: ${originalModel} → ${agent.model}`);
		dispatchToHandler(agent.provider, agent.model, originalModel, req, JSON.stringify(parsedBody), res);
		return;
	}
	const quickModel = quickModels[Math.floor(Math.random() * quickModels.length)];
	if (quickModels.length > 1) {
		log('debug', `[Auto] Random pick from quick models: ${quickModel}`);
	}

	const quickSpec = parseAgentSpec(quickModel);
	if (!quickSpec) {
		log('warn', `[Auto] Invalid quick agent spec: ${quickModel}`);
		const agent = resolveAgent(config, 'default', agentSet);
		if (!agent) {
			res.writeHead(500, { 'Content-Type': 'application/json' });
