...
const handleUserInput = (config, agentSet, sessionKey, originalModel, req, body, res) => {
	const currentMode = getSession(sessionKey) || 'default';
	...
	const quickAgent = agentSet && agentSet.quick;
	if (!quickAgent) {
...
const handleAutoRequest = (provider, targetModel, originalModel, req, body, res, config, sessionId) => {
	...
	if (!userInput) {
		...
		handleUserInput(config, agentSet, sessionKey, originalModel, req, body, res);
	}
};
