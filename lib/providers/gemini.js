...
const handleGeminiRequest = (provider, targetModel, originalModel, req, body, res, onComplete) => {
	...
	if (parsedBody.thinking?.type) {
		const thinking = (parsedBody.thinking.type).toLowerCase();
		if (thinking === 'disabled')
			geminiBody.generationConfig.thinkingLevel = "low";
		else {
			geminiBody.generationConfig.thinkingLevel = "high";
		}
	}
	else {
		geminiBody.generationConfig.thinkingLevel = "low";
	...
	if (req && req.interactionId) {
		logClientStage('claudecode', req.interactionId, '2', 'upstream', geminiBody);
	}
		...
		proxyRes.on('end', () => {
			...
			if (stream) {
				...
				if (req && req.interactionId) {
					...
					logClientStage('claudecode', req.interactionId, '4', 'result', responseBody);
				}
