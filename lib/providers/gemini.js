...
const handleGeminiRequest = (provider, targetModel, originalModel, req, body, res) => {
	...
	if (req && req.interactionId) {
		logClientStage('gemini', req.interactionId, '2', 'upstream', geminiBody);
	}
		...
		proxyRes.on('end', () => {
			...
			if (stream) {
				...
				try {
					...
					if (req && req.interactionId) {
						logClientStage('gemini', req.interactionId, '3', 'response', geminiResp);
					}
					...
					if (req && req.interactionId) {
						logClientStage('gemini', req.interactionId, '4', 'result', anthropicResp);
					}
