		...
		req.on('end', () => {
			...
			if (body && (req.url.includes('/v1/messages'))) {
				try {
					...
					logClientStage('claude', interactionId, '1', 'request', parsed);
					req.interactionId = interactionId;
					
					// Remove Anthropic billing headers from system prompt to prevent cache invalidation
					if (parsed.system) {
						parsed.system = removeBillingHeaderFromList(parsed.system);
					}
					if (Array.isArray(parsed.messages)) {
						parsed.messages.forEach(item => {
