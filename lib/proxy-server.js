...
const createProxyServer = (getConfig) => {
	const server = http.createServer((req, res) => {
		...
		req.on('end', () => {
			...
			if (isOpenAINative) {
				let codexOriginalModel;
				let codexMapped;
				let codexProvider;
				try {
					codexOriginalModel = parsedBody && parsedBody.model;
					if (!codexOriginalModel) {
						res.writeHead(400, { 'Content-Type': 'application/json' });
						res.end(JSON.stringify({ error: { type: 'invalid_request_error', message: 'Missing model parameter' } }));
						return;
					}
					codexMapped = mapModel(codexOriginalModel, config.modelMapping);
					if (!codexMapped) {
						res.writeHead(400, { 'Content-Type': 'application/json' });
						res.end(JSON.stringify({ error: { type: 'invalid_request_error', message: 'No mapping for model: ' + codexOriginalModel } }));
						return;
					}
					codexProvider = config.providers[codexMapped.provider];
					if (!codexProvider) {
						res.writeHead(400, { 'Content-Type': 'application/json' });
						res.end(JSON.stringify({ error: { type: 'invalid_request_error', message: 'Provider not configured: ' + codexMapped.provider } }));
						return;
					}
				}
				catch (e) {
					log('warn', '[Proxy] Codex pre-check failed: ' + e.message);
					res.writeHead(400, { 'Content-Type': 'application/json' });
				...
				const codexSpec = codexMapped.provider + '/' + codexMapped.targetModel;
			...
			if (isGeminiNative) {
				...
				const gemOriginalModel = extractModelFromGeminiURL(req.url);
				if (!gemOriginalModel) {
					res.writeHead(400, { 'Content-Type': 'application/json' });
					res.end(JSON.stringify({ error: { message: 'Cannot extract model from URL', code: 400 } }));
					return;
				}
				const gemMapped = mapModel(gemOriginalModel, config.modelMapping);
				if (!gemMapped) {
					res.writeHead(400, { 'Content-Type': 'application/json' });
					res.end(JSON.stringify({ error: { message: 'No mapping for model: ' + gemOriginalModel, code: 400 } }));
					return;
				}
				const gemProvider = config.providers[gemMapped.provider];
				if (!gemProvider) {
					res.writeHead(400, { 'Content-Type': 'application/json' });
					res.end(JSON.stringify({ error: { message: 'Provider not configured: ' + gemMapped.provider, code: 400 } }));
					return;
				}

				if (gemProvider.type === 'auto') {
				...
				const gemSpec = gemMapped.provider + '/' + gemMapped.targetModel;
				modelRouter.executeWithRetry({
					modelsArray: [gemSpec],
					config,
					buildBody: () => body,
