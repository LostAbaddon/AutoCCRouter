...
const createProxyServer = (getConfig) => {
	const server = http.createServer((req, res) => {
		...
		req.on('end', () => {
			...
			const sessionId = ((() => {
				if (typeof parsedBody.metadata?.user_id === 'string') {
					let json = JSON.parse(parsedBody.metadata?.user_id);
					return json.session_id;
				}
				else {
					return parsedBody.metadata?.user_id?.session_id;
				}
			...
			if (body && (req.url.includes('/v1/messages'))) {
				try {
					...
					if (parsedBody.model) {
						...
						const mapped = mapModel(parsedBody.model, config.modelMapping) || parsedBody.model;
						if (mapped) {
						...
						if (parsedBody.thinking && parsedBody.thinking.type === 'adaptive') {
							const isMinimax = mapped.provider === 'minimax';
							if (isMinimax) {
								log('info', `Thinking: keeping adaptive for MiniMax ${targetModel}`);
							}
							else {
								const hasToolUseWithoutThinking = (parsedBody.messages || []).some((msg) => {
