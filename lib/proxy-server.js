...
const { autoModelRouter } = require('./providers/auto');
...
const getProviderAndModel = (body, url, config, copilotType) => {
	...
	if (modelList.provider === 'auto') {
		...
		targetModel = targetModel.map(model => `${modelList.provider}/${model}`);
...
const createProxyServer = () => {
	const server = http.createServer((req, res) => {
		...
		req.on('end', async () => {
			...
			// 反防护拦截（如果不拦截，可能会导致 Minimax 等 Token Plan 的 Rate Limit 被快速消耗，但 Token 消耗却非常以至于根本看不出）
			...
			// 生成执行函数
			const generateRequestHandler = (handler) => (requestBody, ...args) => new Promise((res, rej) => {
				// 选择模型
				...
				// 根据 provider 和 model 来重新配置请求体
				reqBody.model = selection.model;
				reqBody.max_tokens = resolveMaxTokens(provider, selection.model);
				const searchEngine = isNativeSearchEngine(provider._name);
				console.log('-'.repeat(100), 'SearchEngine:', searchEngine);
				...
				const onDone = (err) => {
					...
					modelRouter.finishTask(provider._name, selection.model, !err, !!err);

					if (err) {
						log('error', `[AutoModelRouter] ${provider._name}/${selection.model} failed: ${err.message}`);
						console.error(err);
						rej(err);
					}
					res();
				...
				// 启动任务
				modelRouter.startTask(provider._name, selection.model);
				handler(provider, selection.model, onDone, reqBody, ...args);
			});
			...
			// Claude Code / Cowork
			if (CopilotType.isClaude) {
				handler = generateRequestHandler(async (provider, model, onDone, requestBody) => new Promise(() => {
					try {
			...
			// Gemini CLI
				...
				handler = generateRequestHandler(async (provider, model, onDone, requestBody) => new Promise((resolve, reject) => {
					try {
